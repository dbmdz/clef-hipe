...
def run_experiment(seed, batch_size, epoch, learning_rate, json_config):
    ...
    # Dataset-related
    dataset_folder = json_config["dataset_folder"]
    ...
    # Corpus
    corpus = ColumnCorpus(data_folder=dataset_folder,
    ...
    dataset = dataset_folder.replace("/", "_").replace(".", "")

    trainer.fine_tune(
        f"histo-flert-fine-tune-{dataset}-{hf_model}-bs{batch_size}-ws{context_size}-e{epoch}-lr{learning_rate}-layers{layers}-crf{use_crf}-{seed}",
